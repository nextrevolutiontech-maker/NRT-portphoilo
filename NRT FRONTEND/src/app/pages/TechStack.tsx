    ...
    <div className="min-h-screen bg-[#F3F4F6] text-[#0F172A] overflow-hidden">
      ...
      <section className="pt-32 pb-40 px-4 sm:px-6 lg:px-12 xl:px-24 bg-[#0F172A] text-white relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl relative z-10">
           <div className="text-[10px] font-black uppercase tracking-[0.4em] text-[#3A5CCC] mb-12">System Architecture</div>
           <h1 className="text-5xl sm:text-[8rem] lg:text-[10rem] font-black tracking-tighter leading-[1] sm:leading-[0.8] mb-12">
              ...
              <span className="font-italic-serif italic font-normal text-transparent bg-clip-text bg-gradient-to-r from-[#3A5CCC] to-[#27324A]">Tech Stack.</span>
           </h1>
               ...
               {stacks.map((stack, i) => (
                 <div key={i} className="grid lg:grid-cols-12 gap-12 lg:gap-24 items-start">
                    <div className="lg:col-span-4 sticky top-32">
                       <div className="w-16 h-16 rounded-2xl bg-[#3A5CCC]/10 flex items-center justify-center text-[#3A5CCC] mb-8 border border-black/5">
                          {stack.icon}
                       </div>
                       <h2 className="text-4xl sm:text-5xl font-black mb-6 tracking-tighter text-[#0F172A] leading-tight">{stack.category}</h2>
                    </div>
                    ...
                    <div className="lg:col-span-8 grid sm:grid-cols-2 gap-6">
                       {stack.items.map((item, j) => (
                         <motion.div 
                           ...
                           viewport={{ once: true }}
                           className="bg-white p-8 rounded-[2rem] border border-black/5 shadow-lg hover:shadow-xl transition-shadow"
                         >
                            <div className="flex items-center gap-3 mb-4">
                               <CheckCircle2 className="w-5 h-5 text-[#14B8A6]" />
                               <h3 className="text-xl font-bold text-[#0F172A]">{item.name}</h3>
                            </div>
                            <p className="text-sm font-semibold text-[#0F172A]/60 leading-relaxed">
                               {item.desc}
                            </p>
                         </motion.div>
                       ))}
