  ...
  useEffect(() => {
    const fetchRecs = async () => {
      try {
        ...
        const recIndex: any = { default: {} };
        
        // Ensure we are accessing the default export if using ES modules
        const data: any = recIndex.default || recIndex;
      ...
      <div className="space-y-10">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {recommendations.continueLearning.slice(0, 4).map((item: any, idx: number) => (
                <Link key={idx} href={`/knowledge/${item.slug}`} className="group p-4 rounded-xl border border-slate-100 bg-slate-50 hover:border-primary hover:shadow-md transition-all">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {recommendations.businessResources.map((item: any, idx: number) => (
                <Link key={idx} href={`/business-problems/${item.slug}`} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-50 text-orange-700 font-semibold hover:bg-orange-600 hover:text-white transition-colors text-sm border border-orange-200 hover:border-orange-600">
                  {item.title}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {recommendations.caseStudies.slice(0, 2).map((item: any, idx: number) => (
                <Link key={idx} href={`/solutions/${item.slug}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition-colors group border border-transparent hover:border-slate-200">
        ...
        <div className="pt-8 border-t border-slate-200">
           <div className="bg-slate-900 rounded-2xl p-6 text-white flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <Link href="/contact" className="shrink-0 bg-primary hover:bg-primary/90 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-colors">
